import asyncio
import json
from model import DecryptedMessage, Patient, User, AccessLevel, serialize_patient, serialize_secret_id_option, Message, serialize_message, AbstractFilter, serialize_abstract_filter, DocIdentifier, RequestedPermission, FilterChain, PaginatedList
...
from crypto import SecretIdOption, ShareMetadataBehaviour, deserialize_simple_share_result, SimpleShareResult, MessageShareOptions
...
class MessageApi:
	...
	def __init__(self, icure_sdk):
		...
		self.encrypted = MessageFlavouredEncryptedApi(executor)
		self.tryAndRecover = MessageFlavouredApi(executor)
	...
	async def with_encryption_metadata_async(self, base: Optional[DecryptedMessage], patient: Optional[Patient], user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = field(default_factory=lambda: SecretIdOption.UseAnySharedWithParent)) -> DecryptedMessage:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			result = None
	...
	def with_encryption_metadata_blocking(self, base: Optional[DecryptedMessage], patient: Optional[Patient], user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = field(default_factory=lambda: SecretIdOption.UseAnySharedWithParent)) -> DecryptedMessage:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"patient": serialize_patient(patient) if patient is not None else None,
			"user": user.__serialize__() if user is not None else None,
	...
	async def share_with_async(self, delegate_id: str, message: DecryptedMessage, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.shareWithAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def share_with_blocking(self, delegate_id: str, message: DecryptedMessage, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
		payload = {
			"delegate_id": delegate_id,
			"message": message.__serialize__(),
			"share_secret_ids": [x0 for x0 in share_secret_ids],
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.shareWithBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def try_share_with_many_async(self, message: DecryptedMessage, delegates: Dict[str, MessageShareOptions]) -> SimpleShareResult:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.tryShareWithManyAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def try_share_with_many_blocking(self, message: DecryptedMessage, delegates: Dict[str, MessageShareOptions]) -> SimpleShareResult:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.tryShareWithManyBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def share_with_many_async(self, message: DecryptedMessage, delegates: Dict[str, MessageShareOptions]) -> DecryptedMessage:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.shareWithManyAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def share_with_many_blocking(self, message: DecryptedMessage, delegates: Dict[str, MessageShareOptions]) -> DecryptedMessage:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.shareWithManyBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def find_messages_by_hc_party_patient_async(self, hc_party_id: str, patient: Patient, start_date: Optional[int] = None, end_date: Optional[int] = None, descending: Optional[bool] = None) -> PaginatedListIterator[DecryptedMessage]:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.findMessagesByHcPartyPatientAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def find_messages_by_hc_party_patient_blocking(self, hc_party_id: str, patient: Patient, start_date: Optional[int] = None, end_date: Optional[int] = None, descending: Optional[bool] = None) -> PaginatedListIterator[DecryptedMessage]:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.MessageFlavouredApi.findMessagesByHcPartyPatientBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
	...
	class MessageFlavouredEncryptedApi:
		...
		async def share_with_async(self, delegate_id: str, message: EncryptedMessage, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
		...
		def share_with_blocking(self, delegate_id: str, message: EncryptedMessage, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			payload = {
				"delegate_id": delegate_id,
				"message": message.__serialize__(),
				"share_secret_ids": [x0 for x0 in share_secret_ids],
	...
	class MessageFlavouredApi:
		...
		async def share_with_async(self, delegate_id: str, message: Message, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
