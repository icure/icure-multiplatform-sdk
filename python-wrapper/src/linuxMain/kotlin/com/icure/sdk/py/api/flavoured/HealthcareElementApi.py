import asyncio
import json
from model import DecryptedHealthElement, Patient, User, AccessLevel, serialize_patient, serialize_secret_id_option, HealthElement, serialize_health_element, AbstractFilter, serialize_abstract_filter, DocIdentifier, IcureStub, RequestedPermission, FilterChain, PaginatedList
...
from crypto import SecretIdOption, ShareMetadataBehaviour, deserialize_simple_share_result, SimpleShareResult, HealthElementShareOptions
...
class HealthcareElementApi:
	...
	async def with_encryption_metadata_async(self, base: Optional[DecryptedHealthElement], patient: Patient, user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = field(default_factory=lambda: SecretIdOption.UseAnySharedWithParent)) -> DecryptedHealthElement:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			result = None
	...
	def with_encryption_metadata_blocking(self, base: Optional[DecryptedHealthElement], patient: Patient, user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = field(default_factory=lambda: SecretIdOption.UseAnySharedWithParent)) -> DecryptedHealthElement:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"patient": serialize_patient(patient),
			"user": user.__serialize__() if user is not None else None,
	...
	def share_with_blocking(self, delegate_id: str, healthcare_element: DecryptedHealthElement, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
		payload = {
			"delegate_id": delegate_id,
			"healthcare_element": healthcare_element.__serialize__(),
			"share_encryption_keys": share_encryption_keys.__serialize__(),
	...
	class HealthcareElementFlavouredEncryptedApi:
		...
		async def share_with_async(self, delegate_id: str, healthcare_element: EncryptedHealthElement, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
		...
		def share_with_blocking(self, delegate_id: str, healthcare_element: EncryptedHealthElement, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			payload = {
				"delegate_id": delegate_id,
				"healthcare_element": healthcare_element.__serialize__(),
				"share_encryption_keys": share_encryption_keys.__serialize__(),
