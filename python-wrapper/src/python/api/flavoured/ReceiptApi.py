...
import json
import base64
from model import DecryptedReceipt, Patient, User, AccessLevel, serialize_patient, serialize_secret_id_option, Receipt, serialize_receipt, EncryptedReceipt, deserialize_receipt, DocIdentifier, RequestedPermission
from model.CallResult import CallResult, create_result_from_json
from kotlin_types import DATA_RESULT_CALLBACK_FUNC, symbols
from ctypes import cast, c_char_p
from typing import Optional, Dict, List
from dataclasses import field
from crypto import SecretIdOption, ShareMetadataBehaviour, deserialize_simple_share_result, SimpleShareResult, ReceiptShareOptions
...
class ReceiptApi:
	...
	class ReceiptFlavouredEncryptedApi:
		...
		async def share_with_async(self, delegate_id: str, receipt: EncryptedReceipt, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
		...
		def share_with_blocking(self, delegate_id: str, receipt: EncryptedReceipt, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			payload = {
				"delegate_id": delegate_id,
				"receipt": receipt.__serialize__(),
				"share_encryption_keys": share_encryption_keys.__serialize__(),
	...
	class ReceiptFlavouredApi:
		...
		async def share_with_async(self, delegate_id: str, receipt: Receipt, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
		...
		def share_with_blocking(self, delegate_id: str, receipt: Receipt, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			payload = {
				"delegate_id": delegate_id,
				"receipt": receipt.__serialize__(),
				"share_encryption_keys": share_encryption_keys.__serialize__(),
