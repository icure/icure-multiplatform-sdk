import asyncio
import json
from model import DocIdentifier, AbstractFilter, serialize_abstract_filter, SubscriptionEventType, EntitySubscriptionConfiguration, EncryptedTopic, FilterChain, PaginatedList, TopicRole
...
class TopicBasicApi:
	...
	async def subscribe_to_events_async(self, events: List[SubscriptionEventType], filter: AbstractFilter, subscription_config: Optional[EntitySubscriptionConfiguration] = None) -> EntitySubscription[EncryptedTopic]:
		...
		loop.run_in_executor(
			self.icure_sdk._executor,
			symbols.kotlin.root.com.icure.sdk.py.subscription.TopicBasicApi.subscribeToEventsAsync,
			self.icure_sdk._native,
			json.dumps(payload).encode('utf-8'),
			callback
		)
	...
	def subscribe_to_events_blocking(self, events: List[SubscriptionEventType], filter: AbstractFilter, subscription_config: Optional[EntitySubscriptionConfiguration] = None) -> EntitySubscription[EncryptedTopic]:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.subscription.TopicBasicApi.subscribeToEventsBlocking(
			self.icure_sdk._native,
			json.dumps(payload).encode('utf-8')
		)
