...
from model.Document import DecryptedDocument, Document, EncryptedDocument
...
from crypto.entities.DocumentShareOptions import DocumentShareOptions
...
class DocumentApi:
	...
	def __init__(self, icure_sdk, executor):
		self.native_api = symbols.kotlin.root.com.icure.sdk.py.api.createDocumentApi()
	...
	async def with_encryption_metadata_async(self, base: Optional[DecryptedDocument], message: Optional[Message], user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = SecretIdOption.UseAnySharedWithParent) -> DecryptedDocument:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			result = None
	...
	def with_encryption_metadata_blocking(self, base: Optional[DecryptedDocument], message: Optional[Message], user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = SecretIdOption.UseAnySharedWithParent) -> DecryptedDocument:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"message": serialize_message(message) if message is not None else None,
			"user": user.__serialize__() if user is not None else None,
	...
	class DocumentFlavouredEncryptedApi:
		...
		def __init__(self, executor):
			self.native_api = symbols.kotlin.root.com.icure.sdk.py.api.createDocumentFlavouredApi()
			self.executor = executor
