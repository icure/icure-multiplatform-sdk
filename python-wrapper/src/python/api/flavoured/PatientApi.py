...
class PatientApi:
	...
	def __init__(self, icure_sdk):
		...
		self.encrypted = PatientFlavouredEncryptedApi(executor)
		self.tryAndRecover = PatientFlavouredApi(executor)
	...
	async def with_encryption_metadata_async(self, base: Optional[DecryptedPatient], user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict)) -> DecryptedPatient:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			result = None
	...
	def with_encryption_metadata_blocking(self, base: Optional[DecryptedPatient], user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict)) -> DecryptedPatient:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"user": user.__serialize__() if user is not None else None,
			"delegates": {k0: v0.__serialize__() for k0, v0 in delegates.items()},
	...
	async def create_delegations_de_anonymization_metadata_async(self, patient: Patient, data_owner_ids: List[str]) -> None:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.createDelegationsDeAnonymizationMetadataAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def create_delegations_de_anonymization_metadata_blocking(self, patient: Patient, data_owner_ids: List[str]) -> None:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.createDelegationsDeAnonymizationMetadataBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def decrypt_patient_id_of_async(self, patient: Patient) -> List[str]:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.decryptPatientIdOfAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def decrypt_patient_id_of_blocking(self, patient: Patient) -> List[str]:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.decryptPatientIdOfBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def register_patient_async(self, hc_party_id: str, group_id: str, token: Optional[str], use_short_token: Optional[bool], create_auto_delegation: bool, patient: DecryptedPatient) -> DataOwnerRegistrationSuccess:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.registerPatientAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def register_patient_blocking(self, hc_party_id: str, group_id: str, token: Optional[str], use_short_token: Optional[bool], create_auto_delegation: bool, patient: DecryptedPatient) -> DataOwnerRegistrationSuccess:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.registerPatientBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def get_patient_id_of_child_document_for_hcp_and_hcp_parents_async(self, child_document: EntityWithTypeInfo, healthcare_party_id: str) -> str:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.getPatientIdOfChildDocumentForHcpAndHcpParentsAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def get_patient_id_of_child_document_for_hcp_and_hcp_parents_blocking(self, child_document: EntityWithTypeInfo, healthcare_party_id: str) -> str:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.getPatientIdOfChildDocumentForHcpAndHcpParentsBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def force_initialise_exchange_data_to_newly_invited_patient_async(self, patient_id: str) -> bool:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.forceInitialiseExchangeDataToNewlyInvitedPatientAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def force_initialise_exchange_data_to_newly_invited_patient_blocking(self, patient_id: str) -> bool:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientApi.forceInitialiseExchangeDataToNewlyInvitedPatientBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def share_with_async(self, delegate_id: str, patient: DecryptedPatient, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.shareWithAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def share_with_blocking(self, delegate_id: str, patient: DecryptedPatient, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
		payload = {
			"delegate_id": delegate_id,
			"patient": patient.__serialize__(),
			"share_secret_ids": [x0 for x0 in share_secret_ids],
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.shareWithBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def try_share_with_many_async(self, patient: DecryptedPatient, delegates: Dict[str, PatientShareOptions]) -> SimpleShareResult:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.tryShareWithManyAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def try_share_with_many_blocking(self, patient: DecryptedPatient, delegates: Dict[str, PatientShareOptions]) -> SimpleShareResult:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.tryShareWithManyBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def share_with_many_async(self, patient: DecryptedPatient, delegates: Dict[str, PatientShareOptions]) -> DecryptedPatient:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.shareWithManyAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def share_with_many_blocking(self, patient: DecryptedPatient, delegates: Dict[str, PatientShareOptions]) -> DecryptedPatient:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.shareWithManyBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def initialise_confidential_secret_id_async(self, patient: DecryptedPatient) -> DecryptedPatient:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.initialiseConfidentialSecretIdAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def initialise_confidential_secret_id_blocking(self, patient: DecryptedPatient) -> DecryptedPatient:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.PatientFlavouredApi.initialiseConfidentialSecretIdBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	class PatientFlavouredEncryptedApi:
		...
		async def share_with_async(self, delegate_id: str, patient: EncryptedPatient, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
		...
		def share_with_blocking(self, delegate_id: str, patient: EncryptedPatient, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			payload = {
				"delegate_id": delegate_id,
				"patient": patient.__serialize__(),
				"share_secret_ids": [x0 for x0 in share_secret_ids],
	...
	class PatientFlavouredApi:
		...
		async def share_with_async(self, delegate_id: str, patient: Patient, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
		...
		def share_with_blocking(self, delegate_id: str, patient: Patient, share_secret_ids: List[str], share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			payload = {
				"delegate_id": delegate_id,
				"patient": patient.__serialize__(),
				"share_secret_ids": [x0 for x0 in share_secret_ids],
