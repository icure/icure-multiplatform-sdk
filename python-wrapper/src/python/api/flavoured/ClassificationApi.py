...
from model.Classification import DecryptedClassification, Classification
...
from crypto.entities.ClassificationShareOptions import ClassificationShareOptions
...
class ClassificationApi:
	...
	def __init__(self, icure_sdk, executor):
		self.native_api = symbols.kotlin.root.com.icure.sdk.py.api.createClassificationApi()
	...
	async def with_encryption_metadata_async(self, base: Optional[DecryptedClassification], patient: Patient, user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = SecretIdOption.UseAnySharedWithParent) -> DecryptedClassification:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			result = None
	...
	def with_encryption_metadata_blocking(self, base: Optional[DecryptedClassification], patient: Patient, user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = SecretIdOption.UseAnySharedWithParent) -> DecryptedClassification:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"patient": serialize_patient(patient),
			"user": user.__serialize__() if user is not None else None,
	...
	class ClassificationFlavouredEncryptedApi:
		...
		def __init__(self, executor):
			self.native_api = symbols.kotlin.root.com.icure.sdk.py.api.createClassificationFlavouredApi()
			self.executor = executor
