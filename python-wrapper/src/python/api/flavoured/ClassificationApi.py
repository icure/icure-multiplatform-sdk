import asyncio
import json
from model import DecryptedClassification, Patient, User, AccessLevel, serialize_patient, serialize_secret_id_option, Classification, serialize_classification, DocIdentifier, RequestedPermission
...
from crypto import SecretIdOption, ShareMetadataBehaviour, deserialize_simple_share_result, SimpleShareResult, ClassificationShareOptions
...
class ClassificationApi:
	...
	def __init__(self, icure_sdk):
		...
		self.encrypted = ClassificationFlavouredEncryptedApi(executor)
		self.tryAndRecover = ClassificationFlavouredApi(executor)
	...
	async def create_classification_async(self, entity: DecryptedClassification) -> DecryptedClassification:
		...
		def make_result_and_complete(success, failure):
			...
			if failure is not None:
				...
				success = DecryptedClassification._deserialize(success.decode('utf-8'))
	...
	async def with_encryption_metadata_async(self, base: Optional[DecryptedClassification], patient: Patient, user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = field(default_factory=lambda: SecretIdOption.UseAnySharedWithParent)) -> DecryptedClassification:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			result = None
	...
	def with_encryption_metadata_blocking(self, base: Optional[DecryptedClassification], patient: Patient, user: Optional[User] = None, delegates: Dict[str, AccessLevel] = field(default_factory=dict), secret_id: SecretIdOption = field(default_factory=lambda: SecretIdOption.UseAnySharedWithParent)) -> DecryptedClassification:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"patient": serialize_patient(patient),
			"user": user.__serialize__() if user is not None else None,
	...
	async def share_with_async(self, delegate_id: str, classification: DecryptedClassification, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			result = None
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.shareWithAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def share_with_blocking(self, delegate_id: str, classification: DecryptedClassification, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
		payload = {
			"delegate_id": delegate_id,
			"classification": classification.__serialize__(),
			"share_encryption_keys": share_encryption_keys.__serialize__(),
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.shareWithBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def try_share_with_many_async(self, classification: DecryptedClassification, delegates: Dict[str, ClassificationShareOptions]) -> SimpleShareResult:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.tryShareWithManyAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def try_share_with_many_blocking(self, classification: DecryptedClassification, delegates: Dict[str, ClassificationShareOptions]) -> SimpleShareResult:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.tryShareWithManyBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def share_with_many_async(self, classification: DecryptedClassification, delegates: Dict[str, ClassificationShareOptions]) -> DecryptedClassification:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.shareWithManyAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def share_with_many_blocking(self, classification: DecryptedClassification, delegates: Dict[str, ClassificationShareOptions]) -> DecryptedClassification:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.shareWithManyBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
	...
	async def find_classifications_by_hc_party_patient_async(self, hc_party_id: str, patient: Patient, start_date: Optional[int] = None, end_date: Optional[int] = None, descending: Optional[bool] = None) -> PaginatedListIterator[DecryptedClassification]:
		...
		loop.run_in_executor(
			self.icure_sdk.executor,
			symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.findClassificationsByHcPartyPatientAsync,
			self.icure_sdk.native,
			json.dumps(payload),
			callback
		)
	...
	def find_classifications_by_hc_party_patient_blocking(self, hc_party_id: str, patient: Patient, start_date: Optional[int] = None, end_date: Optional[int] = None, descending: Optional[bool] = None) -> PaginatedListIterator[DecryptedClassification]:
		...
		call_result = symbols.kotlin.root.com.icure.sdk.py.api.ClassificationFlavouredApi.findClassificationsByHcPartyPatientBlocking(
			self.icure_sdk.native,
			json.dumps(payload)
		)
	...
	async def get_classifications_async(self, entity_ids: List[str]) -> List[DecryptedClassification]:
		...
		def make_result_and_complete(success, failure):
			...
			if failure is not None:
				...
				success = [DecryptedClassification._deserialize(x1) for x1 in success.decode('utf-8')]
				result = CallResult(success=success)
			loop.call_soon_threadsafe(lambda: future.set_result(result))
		payload = {
			"entity_ids": [x0 for x0 in entity_ids],
		...
		async def share_with_many_async(self, classification: EncryptedClassification, delegates: Dict[str, ClassificationShareOptions]) -> EncryptedClassification:
			...
			def make_result_and_complete(success, failure):
				...
				if failure is not None:
					...
					success = EncryptedClassification._deserialize(success.decode('utf-8'))
					result = CallResult(success=success)
				loop.call_soon_threadsafe(lambda: future.set_result(result))
			payload = {
				"classification": classification.__serialize__(),
		...
		async def get_classifications_async(self, entity_ids: List[str]) -> List[EncryptedClassification]:
			...
			def make_result_and_complete(success, failure):
				...
				if failure is not None:
					...
					success = [EncryptedClassification._deserialize(x1) for x1 in success.decode('utf-8')]
					result = CallResult(success=success)
				loop.call_soon_threadsafe(lambda: future.set_result(result))
			payload = {
				"entity_ids": [x0 for x0 in entity_ids],
	...
	class ClassificationFlavouredApi:
		...
		async def share_with_async(self, delegate_id: str, classification: Classification, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			loop = asyncio.get_running_loop()
			future = loop.create_future()
			def make_result_and_complete(success, failure):
				result = None
		...
		def share_with_blocking(self, delegate_id: str, classification: Classification, share_encryption_keys: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), share_owning_entity_ids: ShareMetadataBehaviour = field(default_factory=lambda: ShareMetadataBehaviour.IfAvailable), requested_permission: RequestedPermission = field(default_factory=lambda: RequestedPermission.MaxWrite)) -> SimpleShareResult:
			payload = {
				"delegate_id": delegate_id,
				"classification": classification.__serialize__(),
				"share_encryption_keys": share_encryption_keys.__serialize__(),
		...
		async def share_with_many_async(self, classification: Classification, delegates: Dict[str, ClassificationShareOptions]) -> Classification:
			...
			def make_result_and_complete(success, failure):
				...
				if failure is not None:
					...
					success = Classification._deserialize(success.decode('utf-8'))
					result = CallResult(success=success)
				loop.call_soon_threadsafe(lambda: future.set_result(result))
			payload = {
				"classification": classification.__serialize__(),
		...
		async def get_classifications_async(self, entity_ids: List[str]) -> List[Classification]:
			...
			def make_result_and_complete(success, failure):
				...
				if failure is not None:
					...
					success = [Classification._deserialize(x1) for x1 in success.decode('utf-8')]
					result = CallResult(success=success)
				loop.call_soon_threadsafe(lambda: future.set_result(result))
			payload = {
				"entity_ids": [x0 for x0 in entity_ids],
