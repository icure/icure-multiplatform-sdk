import asyncio
import json
from icure.model import Device, IdWithRev, DeviceAbstractFilter, serialize_abstract_filter, DocIdentifier
...
class DeviceApi:
	...
	async def filter_devices_by_async(self, filter: DeviceAbstractFilter) -> PaginatedListIterator[Device]:
		loop = asyncio.get_running_loop()
		future = loop.create_future()
		def make_result_and_complete(success, failure):
			if failure is not None:
	...
	def filter_devices_by_blocking(self, filter: DeviceAbstractFilter) -> PaginatedListIterator[Device]:
		payload = {
			"filter": serialize_abstract_filter(filter),
		}
