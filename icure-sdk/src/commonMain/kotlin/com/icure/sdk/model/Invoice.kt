...
@Serializable
public sealed interface Invoice : StoredDocument, ICureDocument<String>, Encryptable {
...
public data class EncryptedInvoice(
  ...
  override val invoicingCodes: List<InvoicingCode> = emptyList(),
...
public fun Invoice.copy(
  ...
  securityMetadata: SecurityMetadata? = this.securityMetadata,
                                  ...
                                  return when(this) {
                                      is EncryptedInvoice -> copy(id = id, rev = rev, created =
          ...
          deletionDate = deletionDate, invoiceDate = invoiceDate, sentDate = sentDate, printedDate =
          printedDate, invoicingCodes = invoicingCodes, receipts = receipts, recipientType =
          recipientType, recipientId = recipientId, invoiceReference = invoiceReference,
          thirdPartyReference = thirdPartyReference, thirdPartyPaymentJustification =
          thirdPartyPaymentJustification, thirdPartyPaymentReason = thirdPartyPaymentReason, reason
          = reason, invoiceType = invoiceType, sentMediumType = sentMediumType, interventionType =
          interventionType, groupId = groupId, paymentType = paymentType, paid = paid, payments =
          payments, gnotionNihii = gnotionNihii, gnotionSsin = gnotionSsin, gnotionLastName =
          gnotionLastName, gnotionFirstName = gnotionFirstName, gnotionCdHcParty = gnotionCdHcParty,
          invoicePeriod = invoicePeriod, careProviderType = careProviderType, internshipNihii =
          internshipNihii, internshipSsin = internshipSsin, internshipLastName = internshipLastName,
          internshipFirstName = internshipFirstName, internshipCdHcParty = internshipCdHcParty,
          internshipCbe = internshipCbe, supervisorNihii = supervisorNihii, supervisorSsin =
          supervisorSsin, supervisorLastName = supervisorLastName, supervisorFirstName =
          supervisorFirstName, supervisorCdHcParty = supervisorCdHcParty, supervisorCbe =
          supervisorCbe, error = error, encounterLocationName = encounterLocationName,
          encounterLocationNihii = encounterLocationNihii, encounterLocationNorm =
          encounterLocationNorm, longDelayJustification = longDelayJustification,
          correctiveInvoiceId = correctiveInvoiceId, correctedInvoiceId = correctedInvoiceId,
          creditNote = creditNote, creditNoteRelatedInvoiceId = creditNoteRelatedInvoiceId,
          idDocument = idDocument, admissionDate = admissionDate, locationNihii = locationNihii,
          locationService = locationService, cancelReason = cancelReason, cancelDate = cancelDate,
          options = options, secretForeignKeys = secretForeignKeys, cryptedForeignKeys =
      ...
      is DecryptedInvoice -> copy(id = id, rev = rev, created = created, modified = modified, author
          = author, responsible = responsible, medicalLocationId = medicalLocationId, tags = tags,
          codes = codes, endOfLife = endOfLife, deletionDate = deletionDate, invoiceDate =
          invoiceDate, sentDate = sentDate, printedDate = printedDate, invoicingCodes =
          invoicingCodes, receipts = receipts, recipientType = recipientType, recipientId =
          recipientId, invoiceReference = invoiceReference, thirdPartyReference =
          thirdPartyReference, thirdPartyPaymentJustification = thirdPartyPaymentJustification,
          thirdPartyPaymentReason = thirdPartyPaymentReason, reason = reason, invoiceType =
          invoiceType, sentMediumType = sentMediumType, interventionType = interventionType, groupId
          = groupId, paymentType = paymentType, paid = paid, payments = payments, gnotionNihii =
          gnotionNihii, gnotionSsin = gnotionSsin, gnotionLastName = gnotionLastName,
          gnotionFirstName = gnotionFirstName, gnotionCdHcParty = gnotionCdHcParty, invoicePeriod =
          invoicePeriod, careProviderType = careProviderType, internshipNihii = internshipNihii,
          internshipSsin = internshipSsin, internshipLastName = internshipLastName,
          internshipFirstName = internshipFirstName, internshipCdHcParty = internshipCdHcParty,
          internshipCbe = internshipCbe, supervisorNihii = supervisorNihii, supervisorSsin =
          supervisorSsin, supervisorLastName = supervisorLastName, supervisorFirstName =
          supervisorFirstName, supervisorCdHcParty = supervisorCdHcParty, supervisorCbe =
