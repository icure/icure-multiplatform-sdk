...
@Serializable
sealed interface Topic : StoredDocument, ICureDocument<String>, HasEncryptionMetadata, Encryptable {
...
public fun Topic.copy(
	...
	linkedServices: Set<String> = this.linkedServices,
                                      ...
                                      return when(this) {
                                          is DecryptedTopic -> copy(id = id, rev = rev, created =
          created, modified = modified, healthElementId = healthElementId, contactId = contactId,
          description = description, codes = codes, tags = tags, author = author, responsible =
          responsible, medicalLocationId = medicalLocationId, endOfLife = endOfLife, deletionDate =
          deletionDate, activeParticipants = activeParticipants, securityMetadata =
          securityMetadata, secretForeignKeys = secretForeignKeys, cryptedForeignKeys =
          cryptedForeignKeys, delegations = delegations, encryptionKeys = encryptionKeys,
          encryptedSelf = encryptedSelf, linkedHealthElements = linkedHealthElements, linkedServices
          = linkedServices)
      is EncryptedTopic -> copy(id = id, rev = rev, created = created, modified = modified,
          healthElementId = healthElementId, contactId = contactId, description = description, codes
          = codes, tags = tags, author = author, responsible = responsible, medicalLocationId =
          medicalLocationId, endOfLife = endOfLife, deletionDate = deletionDate, activeParticipants
          = activeParticipants, securityMetadata = securityMetadata, secretForeignKeys =
          secretForeignKeys, cryptedForeignKeys = cryptedForeignKeys, delegations = delegations,
          encryptionKeys = encryptionKeys, encryptedSelf = encryptedSelf, linkedHealthElements =
          linkedHealthElements, linkedServices = linkedServices)
                                          }
