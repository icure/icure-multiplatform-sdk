...
@Serializable
public sealed interface CalendarItem : StoredDocument, ICureDocument<String>, HasEncryptionMetadata,
    Encryptable {
...
public fun CalendarItem.copy(
  ...
  securityMetadata: SecurityMetadata? = this.securityMetadata,
                                      ...
                                      return when(this) {
                                          is DecryptedCalendarItem -> copy(id = id, rev = rev,
          ...
          deletionDate = deletionDate, title = title, calendarItemTypeId = calendarItemTypeId,
          masterCalendarItemId = masterCalendarItemId, patientId = patientId, important = important,
          homeVisit = homeVisit, phoneNumber = phoneNumber, placeId = placeId, address = address,
          addressText = addressText, startTime = startTime, endTime = endTime, confirmationTime =
          confirmationTime, cancellationTimestamp = cancellationTimestamp, confirmationId =
          confirmationId, duration = duration, allDay = allDay, details = details, wasMigrated =
          wasMigrated, agendaId = agendaId, hcpId = hcpId, recurrenceId = recurrenceId, meetingTags
          = meetingTags, flowItem = flowItem, secretForeignKeys = secretForeignKeys,
      ...
      is EncryptedCalendarItem -> copy(id = id, rev = rev, created = created, modified = modified,
          author = author, responsible = responsible, medicalLocationId = medicalLocationId, tags =
          tags, codes = codes, endOfLife = endOfLife, deletionDate = deletionDate, title = title,
          calendarItemTypeId = calendarItemTypeId, masterCalendarItemId = masterCalendarItemId,
          patientId = patientId, important = important, homeVisit = homeVisit, phoneNumber =
          phoneNumber, placeId = placeId, address = address, addressText = addressText, startTime =
          startTime, endTime = endTime, confirmationTime = confirmationTime, cancellationTimestamp =
          cancellationTimestamp, confirmationId = confirmationId, duration = duration, allDay =
          allDay, details = details, wasMigrated = wasMigrated, agendaId = agendaId, hcpId = hcpId,
          recurrenceId = recurrenceId, meetingTags = meetingTags, flowItem = flowItem,
