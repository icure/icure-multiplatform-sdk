...
private suspend fun createTestDataAndApis(): TestData {
	...
	val pLogin = "parent-${UUID.randomUUID()}"
	val pPassword = UUID.randomUUID().toString()
	val pPrivateKey =
	...
	val aPassword = UUID.randomUUID().toString()
	val aPrivateKey =
	...
	val bPassword = UUID.randomUUID().toString()
	val bPrivateKey =
...
class LegacyMetadataMigrationTest : StringSpec({
	 ...
	 */
	"Sharing data as a child of a parent with legacy access should work" {
		val testData = createTestDataAndApis().also { println(it) }
		val apiA = testData.a.api()
		val apiB = testData.b.api()
		...
		secretIdsKnownByB shouldNotContain testData.patientConfidentialSecretId
		...
		secureDelegations.find { it.delegator == testData.b.dataOwnerId && it.delegate == testData.p.dataOwnerId }.shouldNotBeNull()
		val sharedPatientWithoutLegacyDetails = sharedPatient.copy(delegations = emptyMap(), encryptionKeys = emptyMap())
		...
		val secureDelegationAtoX2 = secureDelegations2.find { it.delegator == testData.a.dataOwnerId && it.delegate == x2.dataOwnerId }.shouldNotBeNull()
		secureDelegationAtoX2.parentDelegations shouldHaveSize 2
		apiB.patient.getSecretIdsOf(sharedPatient2) shouldContainExactlyInAnyOrder secretIdsKnownByB
	}
	...
	"sharing data created with legacy api by the same user" {
		...
		secureDelegations.find { it.delegator == testData.a.dataOwnerId && it.delegate == testData.p.dataOwnerId }.shouldNotBeNull()
		secureDelegations.find { it.delegator == testData.a.dataOwnerId && it.delegate == x.dataOwnerId }.shouldNotBeNull()
		val sharedPatientWithoutLegacyDetails = sharedPatient.copy(delegations = emptyMap(), encryptionKeys = emptyMap())
		apiA.patient.getSecretIdsOf(sharedPatientWithoutLegacyDetails) shouldBe secretIdsKnownByA
		apiB.patient.getSecretIdsOf(sharedPatient) shouldBe secretIdsKnownByB
		apiB.patient.getSecretIdsOf(sharedPatientWithoutLegacyDetails) shouldBe secretIdsKnownByB
		apiX.patient.getSecretIdsOf(sharedPatient) shouldBe secretIdsKnownByB
	}
})
