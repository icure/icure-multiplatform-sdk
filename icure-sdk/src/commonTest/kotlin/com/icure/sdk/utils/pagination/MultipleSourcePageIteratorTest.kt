...
class MultipleSourcePageIteratorTest : StringSpec({
	...
	fun createIterator(): Pair<MultipleSourcePageIterator<Datum, Int>, List<Datum>> {
		...
		val firstPages = Array<PaginatedList<Datum>>(7) {
			if(it <= 2) PaginatedList(nextKeyPair = PaginatedDocumentKeyIdPair(startKey = JsonPrimitive("second_page"), startKeyDocId = "some id"))
			else PaginatedList()
		}
		...
		val secondPages = Array<PaginatedList<Datum>>(3) {
			if(it == 0) PaginatedList(nextKeyPair = PaginatedDocumentKeyIdPair(startKey = JsonPrimitive("third_page"), startKeyDocId = "some id"))
			else PaginatedList()
		}
		...
		val thirdPages = Array(1) { PaginatedList(rows = thirdPagesElements) }
		...
		val iterator = MultipleSourcePageIterator(
			...
			when {
				...
				nextKeyPair?.startKey?.jsonPrimitive?.content == "second_page" -> {
					secondPages[params]
				}
				nextKeyPair?.startKey?.jsonPrimitive?.content == "third_page" -> {
					thirdPages[params]
				}
	...
	"A MultipleSourceIterator will ignore any duplicate based on the id" {
		...
		val firstPage = PaginatedList(rows = listOf(elements[0], elements[0], elements[1], elements[1], elements[3]))
		val secondPages = listOf(
			PaginatedList(
				rows = listOf(elements[0], elements[0], elements[0], elements[1], elements[2]),
				nextKeyPair = PaginatedDocumentKeyIdPair(JsonPrimitive("key"), "id")
			),
		...
		val iterator = MultipleSourcePageIterator(
			...
			when {
				...
				nextKeyPair?.startKey?.jsonPrimitive?.content == "key" -> secondPages.last()
