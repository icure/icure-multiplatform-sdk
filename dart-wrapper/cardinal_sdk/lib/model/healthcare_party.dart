...
class HealthcareParty implements StoredDocument, Named, Person, CryptoActor, DataOwner, HasCodes, HasTags {
	...
	factory HealthcareParty.fromJSON(Map<String, dynamic> data) {
		return HealthcareParty(
			...
			statuses: data["statuses"].map((x0) => HealthcarePartyStatus.fromJSON(x0) ),
			statusHistory: data["statusHistory"].map((x0) => HealthcarePartyHistoryStatus.fromJSON(x0) ),
			specialityCodes: data["specialityCodes"].map((x0) => CodeStub.fromJSON(x0) ),
			...
			flatRateTarifications: data["flatRateTarifications"].map((x0) => DecryptedFlatRateTarification.fromJSON(x0) ),
