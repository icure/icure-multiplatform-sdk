...
class DecryptedMessage implements Message {
	...
	factory DecryptedMessage.fromJSON(Map<String, dynamic> data) {
		return DecryptedMessage(
			...
			recipients: data["recipients"].map((x0) => x0 ),
			toAddresses: data["toAddresses"].map((x0) => x0 ),
			...
			messageAttachments: data["messageAttachments"].map((x0) => MessageAttachment.fromJSON(x0) ),
			transportGuid: data["transportGuid"],
			remark: data["remark"],
			conversationGuid: data["conversationGuid"],
			subject: data["subject"],
			invoiceIds: data["invoiceIds"].map((x0) => x0 ),
			parentId: data["parentId"],
			externalRef: data["externalRef"],
			unassignedResults: data["unassignedResults"].map((x0) => x0 ),
